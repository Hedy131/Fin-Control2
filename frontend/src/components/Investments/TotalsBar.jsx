...
export default function TotalsBar({ positions }) {
  ...
  for (const p of positions) {
    if (!byCurrency[p.currency]) byCurrency[p.currency] = { invested: 0, current: 0 }
    byCurrency[p.currency].invested += p.invested_amount || 0
    byCurrency[p.currency].current += p.current_value || 0
  }
      ...
      {rows.map(([currency, totals]) => {
        ...
        return (
          <div key={currency} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <p className="text-sm text-gray-500 mt-2">Valor Atual</p>
            <p className="text-lg font-bold text-gray-900">{formatCurrency(totals.current, currency)}</p>
            <p className={`text-sm font-semibold mt-2 ${gain >= 0 ? 'text-green-600' : 'text-red-600'}`}>
