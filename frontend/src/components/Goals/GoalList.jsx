...
function GoalCard({ goal, category, accounts, onSave, highlighted }) {
...
export default function GoalList({ goals, categories, accounts, onSave, highlightId }) {
  if (goals.length === 0) {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {goals.map((g) => (
        <GoalCard
          key={g.id}
          goal={g}
          category={categoryFor(g.category_id)}
          accounts={accounts}
          onSave={onSave}
          highlighted={g.category_id === highlightId}
        />
      ))}
    </div>
  )
